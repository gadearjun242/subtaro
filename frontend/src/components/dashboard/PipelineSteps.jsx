  ...
  return (
    <ol className="space-y-1">
      {steps.map((step, i) => {
        ...
        return (
          <li key={step.stepNumber} className="relative flex gap-3 pb-5 last:pb-0">
            {!isLast && (
              <span
                className={clsx(
                  'absolute left-[11px] top-6 h-full w-px',
                  step.status === 'success' ? 'bg-emerald-300 dark:bg-emerald-800' : 'bg-slate-200 dark:bg-slate-800'
                )}
              />
            )}
            <Icon className={clsx('mt-0.5 h-[22px] w-[22px] shrink-0', meta.cls, meta.spin && 'animate-spin')} />
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 flex flex-wrap items-center gap-x-2 text-xs text-slate-400">
                ...
                {step.durationSeconds != null && step.status === 'success' && (
                  <span>Completed in {formatDuration(step.durationSeconds)}</span>
                )}
