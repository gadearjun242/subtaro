...
const MODES = {
  ...
  selectable: {
    ...
    tagline: 'One video file, toggle captions on or off',
    points: [
      'Viewers toggle captions on or off themselves',
      'No re-encoding — added in seconds, not minutes',
      'Stays a single .mp4 — nothing extra to store or download',
    ],
  },
}
    ...
    <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-slate-500 dark:text-slate-400">
          Choose per project, and change your mind later — editing a subtitle can switch modes
          and re-render the video automatically.
        </p>
      </div>
      ...
      <div className="mx-auto mt-10 grid max-w-4xl gap-8 lg:grid-cols-2 lg:items-center">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <h3 className="mt-4 text-xl font-bold text-slate-900 dark:text-white">{mode.title}</h3>
            <p className="mt-1 text-sm font-medium text-brand-600 dark:text-brand-400">{mode.tagline}</p>
          ...
          <div className="relative flex aspect-video items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800">
            <AnimatePresence mode="wait">
              {active === 'embedded' ? (
                ...
                >
                  <ListVideo className="h-3.5 w-3.5" /> CC <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />
                </motion.div>
              )}
            </AnimatePresence>
